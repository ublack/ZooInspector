...
public class ZooInspectorManagerImpl implements ZooInspectorManager {
    ...
    public List<String> getChildren(String nodePath) {
        if (connected) {
            try {
                List<String> cacheObj = cache.get(nodePath);
                if (cacheObj != null) {
                    return cacheObj;
                }
                List<String> children = zooKeeper.getChildren(nodePath, false);
                Collections.sort(children);
                cache.put(nodePath, children);
                return children;
            } catch (Exception e) {
    ...
    public String getNodeChild(String nodePath, int childIndex) {
        if (connected) {
            try {
                List<String> cacheObj = cache.get(nodePath);
                if (cacheObj != null) {
                    return cacheObj.get(childIndex);
                }
                Stat s = zooKeeper.exists(nodePath, false);
                if (s != null) {
    ...
    public int getNodeIndex(String nodePath) {
        if (connected) {
            ...
            List<String> cacheObj = cache.get(parentPath);
            if (cacheObj != null) {
                cacheObj.indexOf(nodePath);
            }
                ...
                for (String nodeElement : nodeElements) {
                    ...
                    if (s == null) {
                        zooKeeper.create(node, this.encryptionManager
                                .encryptData(null), Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
                        parent = node;
                    }
                }
                return true;
            ...
            for (String node : selectedNodes) {
                if (!watchers.containsKey(node)) {
                    try {
                        ...
                        LoggerFactory.getLogger().error("Error occurred adding node watcher for node: " + node, e);
                    }
                }
            }
        }
    ...
    private void loadDefaultConnectionFile() throws IOException {
        if (defaultConnectionFile.exists()) {
            ...
            try {
                ...
                reader.close();
                setLastConnectionProps(props); // 启动时,获取.
            }
